// Port of app/admin/payments/page.tsx. The original was an async server
...
export default function AdminPaymentsPage() {
  ...
  const stats = [
    { label: "Total Value", value: `₹${(total / 100).toLocaleString()}`, icon: IndianRupee, tint: "bg-[#111827] text-white" },
            ...
            <tbody className="divide-y divide-neutral-100">
              {payments.map((p: any) => (
                <tr key={p.id} className="hover:bg-neutral-50/60 transition-colors">
                  <td className="px-6 py-4 font-medium text-[#111827]">{p.userEmail}</td>
                  <td className="px-6 py-4 font-bold text-[#111827]">₹{(p.total / 100).toFixed(2)}</td>
