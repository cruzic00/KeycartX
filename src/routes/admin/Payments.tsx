...
export default function AdminPaymentsPage() {
  ...
  const stats = [
    { label: "Total Value", value: `₹${(total / 100).toLocaleString()}`, icon: IndianRupee, tint: "bg-[#1f2937] text-white" },
            ...
            <tbody className="divide-y divide-neutral-100">
              {payments.map((p: any) => (
                <tr key={p.id} className="hover:bg-neutral-50/60 transition-colors">
                  <td className="px-6 py-4 font-medium text-[#1f2937]">{p.userEmail}</td>
                  <td className="px-6 py-4 font-bold text-[#1f2937]">₹{(p.total / 100).toFixed(2)}</td>
