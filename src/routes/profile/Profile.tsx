...
export default function AccountPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto py-10 px-2">
      <h1 className="text-3xl font-bold text-[#111827]">Hello, {name}</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
        {cards.map((c, i) =>
          c.href.startsWith("mailto:") ? (
            ...
            >
              <div className="p-3 rounded-xl bg-neutral-100 text-[#1f2937] group-hover:bg-[#111827] group-hover:text-white transition-colors shrink-0">
                <c.icon size={24} />
              </div>
              <div className="flex-1 min-w-0">
                <h3 className="font-bold text-[#111827] flex items-center gap-1">
                  {c.title}
